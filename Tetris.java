...
public class Tetris extends JPanel {
    ...
    private void paintState(Graphics g) {
        if (game_state == PLING) {
            g.drawString(show_state[PLING], 370, 475);
        } else if (game_state == STOP) {
            g.drawString(show_state[STOP], 370, 475);
        } else {
            g.drawString(show_state[OVER], 370, 475);
            g.setColor(Color.RED);
            g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 40));
            g.drawString("GAME OVER!", 22, 290);
        }
    }

    private void paintSource(Graphics g) {
        g.setFont(new Font(Font.SANS_SERIF, Font.BOLD, 30));
        g.drawString("分数: " + totalScore, 370, 185);
        g.drawString("行数: " + totalLine, 370, 320);
    }

    private void paintNextOne(Graphics g) {
        Cell[] cells = nextOne.cells;
        for (Cell cell : cells) {
            int x = cell.getCol() * CELL_SIZE + 272;
